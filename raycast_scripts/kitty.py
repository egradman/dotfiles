...
match sys.argv[1].lower():
    ...
    case "acu":
        cmd = '''kitten @ launch --to=unix:`ls /tmp/kitty-* | tail -1` --type=tab --tab-title "ACU" --color background="#220000" -- kitten ssh -t rsadmi@192.168.2.1 zsh -l -c "tmux -u new-session -A -s main"  '''

    case "credence":
        cmd = '''kitten @ launch --to=unix:`ls /tmp/kitty-* | tail -1` --type=tab --tab-title "credence" --color background="#220000" -- kitten ssh -t rsadmi@192.168.8.8 zsh -l -c "tmux -u new-session -A -s main"  '''
